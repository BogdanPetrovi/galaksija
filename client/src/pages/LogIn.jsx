  ...
  return (
    <div className='h-screen w-screen flex justify-center items-center'>
      <div className="md:h-1/4 h-1/2 2xl:w-2/4 w-5/6 bg-secondary rounded-2xl flex flex-col justify-center items-center" >
        ...
        <input type="text" className='pl-2 w-2/3 h-12 text-2xl text-black rounded-md mb-4' maxLength={17} onChange={(ev) => setMac(ev.target.value.toUpperCase())} value={mac} onKeyDown={e => { if(e.key === 'Enter'){submit()} } } />
        <div className='flex flex-col w-full justify-center items-center'>
          <input type="submit" value="Potvrdi" className='md:w-1/4 w-1/2 h-14 bg-slate-900 rounded-lg cursor-pointer select-none text-xl' onClick={submit} />
